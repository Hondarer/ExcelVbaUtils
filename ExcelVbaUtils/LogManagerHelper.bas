...
Public Sub LogManagerHelper_LocalInit()
    ...
    ' ログファイル名の組み立て
    apLogFile.filePath = ThisWorkbook.Path & "\" & RemoveExtension(ThisWorkbook.Name) & "_dbg.log"
    ...
    ' 操作ログ用ファイル名の組み立て
    apOperationLogFile.filePath = ThisWorkbook.Path & "\" & RemoveExtension(ThisWorkbook.Name) & "_ope.log"
